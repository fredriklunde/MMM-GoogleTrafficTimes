        ...
        service.getDistanceMatrix({
          ...
          if (status !== 'OK') {
			...
			if (results[j].duration_in_traffic.value > results[j].duration.value) {trafficIcon = 'textWhenLate'};
                        if (destinationList[j].match(re1)) {wrapper.innerHTML += '<span class="' + trafficIcon + '"><span>' +  name1 + ' - ' + results[j].duration_in_traffic.text + '</span></span>'};
                        if (destinationList[j].match(re2)) {wrapper.innerHTML += '<span class="' + trafficIcon + '"><span>' +  name2 + ' - ' + results[j].duration_in_traffic.text + '</span></span>'};
                        if (destinationList[j].match(re3)) {wrapper.innerHTML += '<span class="' + trafficIcon + '"><span>' +  name3 + ' - ' + results[j].duration_in_traffic.text + '</span></span>'};
              }
            }
          }
        });
